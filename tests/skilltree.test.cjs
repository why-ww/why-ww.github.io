...
for (const id of Object.keys(data.pages)) {
 ...
 assert.equal(empty.status(id).state,id==='root'?'learning':'unlearned');
 assert.equal(empty.path(id)[0].id,'root');
}
assert.deepEqual(empty.status('root/签到'),{done:1,total:1,state:'mastered'});
assert.equal(empty.status('root').done,1);
assert.equal(empty.articles.has('root/签到'),false);
...
assert.equal(partial.status('root').done,2);
...
assert.equal(duplicate.status('root').done,2);
...
assert.equal(createModel(data,[article('root/签到')]).status('root').done,1);
